...
///
/// Uses [`OsRng`] for cryptographically secure randomness. The result is safe
/// for use as an `API` key or admin token; store only its [`lookup_hash`] and
/// [`hash_argon2`] output, never the raw secret.
#[must_use]
